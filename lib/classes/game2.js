...
Game.prototype.createBonus = function () {
    ...
    // Condiciones según la cantidad de bolitas restantes
    if ((this.world.balls.remaining == 240 || this.world.balls.remaining == 200) && !this.prize_launched) {
        // Asignación de fruta según las bolitas comidas
        if (this.world.balls.remaining == 240) { // Primer grupo de bolitas
            fruit = 3; // Ejemplo: fruta 3
        } else if (this.world.balls.remaining == 200) { // Segundo grupo de bolitas
            fruit = 4; // Ejemplo: fruta 4
        } else {
            fruit = 10; // Fruta por defecto en caso de que no entre en las anteriores (ajústala si es necesario)
        }
    
        // Encuentra una posición aleatoria válida en el mapa
        do {
            tile = [Math.floor(Math.random() * NROWS), Math.floor(Math.random() * NCOLS)];
        } while (
            this.world.logical_map[tile[0] * NCOLS + tile[1]] == 0 || 
            this.world.balls.matrix[tile[0] * NCOLS + tile[1]] != 0
        );
    
        // Asigna la fruta al mapa
        this.world.balls.matrix[tile[0] * NCOLS + tile[1]] = fruit;
        this.prize_launched = true;

        // Configura la eliminación del bonus después de 8 segundos
